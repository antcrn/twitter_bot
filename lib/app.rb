...
def tweet(str)
    login_twitter.update(str)
end

################
#   Tweeter (X) random journalist
################

def random_journalists(num)
     ...
     to_arr=arr.sample(num)
     to_arr.each {|k|
      login_twitter.update("Merci pour ce que vous faites #{k} #bonjour_monde @the_hacking_pro")
    }
end

################
#   Like a str or hasthag latest tweet
################

def favorite(str)
login_twitter.search(str, result_type:"recent").take(25).collect do |tweet|
...
def follow(str)
  login_twitter.search(str, result_type:"recent").take(20).collect do |tweet|
...
################
#   Follow and like tweet contain a string
################

def live_follow_favorite(topics)
  login_twitter_streaming.filter(track: topics) do |object|
    login_twitter.follow(object.user) if object.is_a?(Twitter::Tweet)
    login_twitter.favorite(object) if object.is_a?(Twitter::Tweet)
    end
end
